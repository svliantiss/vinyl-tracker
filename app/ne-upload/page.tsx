...
export default function NeUploadPage() {
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Process each file sequentially but with optimized analysis
      for (let i = 0; i < files.length; i++) {
        ...
        // For fast analysis, we can analyze just a portion of longer tracks
        // This makes the analysis much faster for longer files
        let analysisBuffer = audioBuffer;
        
        // For files longer than 30 seconds, analyze just the first 30 seconds
        // Most songs have established their tempo by this point
        if (duration > 30) {
          // Create a shorter buffer with just the first 30 seconds
          const shorterDuration = 30;
          const sampleRate = audioBuffer.sampleRate;
          const shorterLength = Math.floor(shorterDuration * sampleRate);
          
          // Create a new buffer with the same sample rate
          ...
          const shorterBuffer = tempContext.createBuffer(
            audioBuffer.numberOfChannels,
            shorterLength,
            sampleRate
          );
          
          // Copy data from the original buffer to our shorter buffer
          for (let channel = 0; channel < audioBuffer.numberOfChannels; channel++) {
            const originalData = audioBuffer.getChannelData(channel);
            const shorterData = shorterBuffer.getChannelData(channel);
            
            for (let i = 0; i < shorterLength; i++) {
              shorterData[i] = originalData[i];
            }
          }
          
          analysisBuffer = shorterBuffer;
        }
        ...
        try {
          ...
          const offlineContext = new OfflineAudioContext(
            analysisBuffer.numberOfChannels,
            analysisBuffer.length,
            analysisBuffer.sampleRate
          );
          ...
          // Use realtime-bpm-analyzer to analyze the full buffer with the filtered audio
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-black min-h-screen">
      ...
      <header className="flex items-center p-4 border-b border-white/10">
        ...
        <h1 className="text-xl text-white/90 font-medium ml-4">Quick BPM Analyzer</h1>
      </header>
